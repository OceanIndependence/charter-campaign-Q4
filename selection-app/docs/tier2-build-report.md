...
| Globe engine | `src/lib/atlas/globe.ts` via `src/components/atlas/AtlasGlobe.tsx` | The Tier 2 globe stage. Two additive changes: the wrapper now accepts `home` (the engine already did) and `palette`. Tier 2 passes `LIGHT_PALETTE` from `src/lib/atlas/palette.ts` (light sea, darker land, ink labels, ringed mint dots); Tier 1 keeps the default dark palette. |
